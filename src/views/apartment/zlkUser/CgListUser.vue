...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    ...
    >
      ...
      <cg-table-column prop="userNo" :page="paginationCurrentPage" :label="$t('zlkUser.field.userNo')" sortable align="left" >
        <template slot-scope="scope">
          {{ scope.row.userNo }}
        </template>

      </cg-table-column>
      <el-table-column prop="realName" width="128" :label="$t('zlkUser.field.realName')" sortable align="left" >
        <template slot-scope="scope">
          {{ scope.row.realName }}
        </template>

      </el-table-column>
      <cg-table-column prop="userType" type="dict" :page="paginationCurrentPage" :label="$t('zlkUser.field.userType')" align="right" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.userType,dictionary.dictUserType,false,true) }}
        </template>

      </cg-table-column>
      <cg-table-column prop="idType" type="dict" :page="paginationCurrentPage" :label="$t('zlkUser.field.idType')" align="right" >
        <template slot-scope="scope">
      ...
      <el-table-column prop="area" type="dict" :show-overflow-tooltip="true" width="50" :label="$t('zlkUser.field.area')" align="left" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.area,dictionary.dictArea,false,true) }}
        </template>

      </el-table-column>
      <el-table-column prop="sex" type="dict" width="128" :label="$t('zlkUser.field.sex')" sortable align="left" >
        <template slot-scope="scope">
      ...
      <cg-table-column prop="createTime" type="datetime" :page="paginationCurrentPage" :label="$t('zlkUser.field.createTime')" sortable align="left" >
        <template slot-scope="scope">
      ...
      <cg-table-column prop="userPassword" :page="paginationCurrentPage" v-if="false" :label="$t('zlkUser.field.userPassword')" align="left" >
        <template slot-scope="scope">
    ...
    <cg-query-condition v-model="showQuery" ref="query" :modal="!joinMode" :queryRecord="queryRecord"
      ...
      <el-form-item v-show="queryRecord.search" :label="$t('system.action.field')" prop="searchFields" :size="$store.state.app.size">
        <cg-select v-model="queryRecord.searchFields" dictionary="userNo|zlkUser.field.userNo,realName|zlkUser.field.realName,idNumber|zlkUser.field.idNumber,sex|zlkUser.field.sex,phone|zlkUser.field.phone,employeeNo|zlkUser.field.employeeNo,createTime|zlkUser.field.createTime," multiple/>
      </el-form-item>
      <el-divider />
      <div v-show="!queryRecord.search">
        <el-form-item :label="$t('zlkUser.field.userNo')" prop="userNo" :size="$store.state.app.size">
          <el-input v-model="queryRecord.userNo" type="text" name="userNo"
                    :readonly="fixedQueryRecord.userNo?true:false" :label="$t('zlkUser.field.userNo')" clearable resize autofocus/>
        </el-form-item>
        ...
        <el-form-item :label="$t('zlkUser.field.sex')" prop="sex" :size="$store.state.app.size">
        ...
        <el-form-item :label="$t('zlkUser.field.createTime')" prop="createTime" :size="$store.state.app.size">
          <cg-date-picker v-model="queryRecord.createTime" :title="$t('zlkUser.field.createTime')" name="createTime" :align="mobile?'right':'center'" type="datetimerange" :picker-options="datePickerOptions()"
                          :readonly="fixedQueryRecord.createTime?true:false" format="yyyy-MM-dd HH:mm" clearable />
        </el-form-item>
      </div>
    </cg-query-condition>
  </div>
...
const Comp = {
  ...
  data() {
    return {
      ...
      queryRecordFields: ['userNo','realName','idNumber','sex','phone','employeeNo','createTime'],
  ...
  computed: {
    ...
    allActions() {
      ...
      else return 'query,,list,add,view,edit,import,export,sample,deleteUser,downloadSysUser,'
    }
  },
  mounted() {
