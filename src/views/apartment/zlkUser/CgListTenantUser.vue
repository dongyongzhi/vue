...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-tenantuser .el-table__body-wrapper" :visibility-height="200">
    ...
    <nut-scroller v-if="mobile && isTableMode()"
                  wrapperElement=".cg-list-tenantuser .el-table__body-wrapper"
                  :isLoading="listLoading"
    ...
    <cg-query-condition v-model="showQuery" ref="query" :modal="!joinMode" :queryRecord="queryRecord"
      ...
      <el-form-item v-show="queryRecord.search" :label="$t('system.action.field')" prop="searchFields" :size="$store.state.app.size">
        <cg-select v-model="queryRecord.searchFields" dictionary="userNo|zlkUser.field.userNo,realName|zlkUser.field.realName,idNumber|zlkUser.field.idNumber,sex|zlkUser.field.sex,phone|zlkUser.field.phone," multiple/>
      </el-form-item>
      <el-divider />
...
const Comp = {
  ...
  data() {
    return {
      ...
      queryRecordFields: ['userNo','realName','idNumber','sex','phone'],
  ...
  computed: {
    ...
    allActions() {
      ...
      else return 'query,,list,add,view,edit,import,export,sample,deleteUser,'
    }
  },
  mounted() {
...
const mixins = [Comp]
const mixinContext = require.context('.', false, /CgListTenantUser-mixin\.(js|vue)$/)
