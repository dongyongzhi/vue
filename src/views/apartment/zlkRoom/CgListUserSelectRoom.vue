...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-userselectroom .el-table__body-wrapper" :visibility-height="200">
    ...
    >
      ...
      <cg-table-column prop="roomName" :page="paginationCurrentPage" :label="$t('zlkRoom.field.roomName')" sortable align="left" >
        <template slot-scope="scope">
          {{ scope.row.roomName }}
        </template>

      </cg-table-column>
      <cg-table-column prop="addressId" type="dict" :page="paginationCurrentPage" :label="$t('zlkRoom.field.addressId')" align="left" >
        <template slot-scope="scope">
          {{ dictValue(scope.row.addressId,dictionary.dictAddressId,false,true) }}
        </template>

      </cg-table-column>
      <cg-table-column prop="regTime" type="datetime" :page="paginationCurrentPage" :label="$t('zlkRoom.field.regTime')" align="left" >
        <template slot-scope="scope">
    ...
    <nut-scroller v-if="mobile && isTableMode()"
                  wrapperElement=".cg-list-userselectroom .el-table__body-wrapper"
                  :isLoading="listLoading"
...
const mixins = [Comp]
const mixinContext = require.context('.', false, /CgListUserSelectRoom-mixin\.(js|vue)$/)
