<template>
  <div class="cg-list">
    <el-card shadow="hover">
      ...
      <CgListTenantRoomMapping ref="cgList" :height="clientHeight" @detail="doShowDetail" :fixedQueryRecord="fixedQueryRecord"/>
    </el-card>
  </div>
</template>

<script>
import CgListTenantRoomMapping from './CgListTenantRoomMapping.vue'
import ParentList from '@/views/common-views/components/list'
const Comp = {
  name: 'TenantRoomMappingList',
  components: { CgListTenantRoomMapping },
  mixins: [ParentList],
  data() {
    return {
