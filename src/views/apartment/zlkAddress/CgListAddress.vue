...
<template>
  <div :class="joinMode?'cg-join-list':'cg-list'">
    <el-backtop v-if="isTableMode()" ref="cgListBacktop" class="el-backtop" target=".cg-list-address .el-table__body-wrapper" :visibility-height="200">
    ...
    >
      ...
      <el-table-column prop="name" width="300" :label="$t('zlkAddress.field.name')" sortable align="left" >
    ...
    <cg-query-condition v-model="showQuery" ref="query" :modal="!joinMode" :queryRecord="queryRecord"
      ...
      <el-form-item v-show="queryRecord.search" :label="$t('system.action.field')" prop="searchFields" :size="$store.state.app.size">
        <cg-select v-model="queryRecord.searchFields" dictionary="name|zlkAddress.field.name," multiple/>
      </el-form-item>
      <el-divider />
      <div v-show="!queryRecord.search">
        <el-form-item :label="$t('zlkAddress.field.name')" prop="name" :size="$store.state.app.size">
          <el-input v-model="queryRecord.name" type="text" name="name"
                    :readonly="fixedQueryRecord.name?true:false" :label="$t('zlkAddress.field.name')" clearable resize autofocus/>
        </el-form-item>
      </div>
    </cg-query-condition>
  </div>
...
const Comp = {
  ...
  data() {
    return {
      ...
      formPath: '/apartment/zlkAddress/record',
      parentField: 'parent',
      idField: 'id',
      dictionary: {
        dictParent: []
  ...
  computed: {
    additionalActions() {
      if (this.joinMode) return []
      else return [
        {
          ...
          title: 'zlkAddress.action.importRoom',
          groupid: 10,
          confirm: '',
          rowProperty: 'nr',
          actionProperty: 'go',
          displayProperties: 'tb',
          appendClass: '{url:"/apartment/zlkAddress/import",openMode:"add"}',
          needRefresh: false
        },
      ]
    },
    allActions() {
      ...
      else return 'query,,list,add,edit,delete,import,export,importRoom,'
    }
  },
  mounted() {
...
const mixins = [Comp]
const mixinContext = require.context('.', false, /CgListAddress-mixin\.(js|vue)$/)
